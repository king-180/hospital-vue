    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/outpatientdoctor/checktemplate/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.name = data.checktemplate.name
                this.dataForm.userid = data.checktemplate.userid
                this.dataForm.creationtime = data.checktemplate.creationtime
                this.dataForm.scope = data.checktemplate.scope
                this.dataForm.recordtype = data.checktemplate.recordtype
                this.dataForm.delmark = data.checktemplate.delmark
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/outpatientdoctor/checktemplate/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
